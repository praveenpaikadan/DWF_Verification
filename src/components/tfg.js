...
export const TFG = ({TFGData}) => {
 
  
    const [activeProfileID, setActiveProfileID] = useState(TFGData[0].profile_number)
    ...
    return (
        <div style={{flex: 1, display: 'flex', flexDirection: 'row'}}>
            <div style={{display: 'flex', flexDirection: 'column'}}>
                <div style={{alignSelf: 'flex-start', display: 'flex', flexDirection: 'row'}}>
                    <select style={{width: 220, height: 50}} value={activeProfileID} onChange={(e)=>(setActiveProfileID(e.target.value))}>
                        {TFGData.map((item, index) => <option key={String(item.profile_number)} value={item.profile_number}>{`${item.profile_number} ${item.profile_name}`}</option>)}      
                    </select>
                    <div style={{display: 'flex', flexDirection: 'column', alignItems:'flex-end'}}>
                        <label style={{width: 80, textAlign:'right' }}>PCC</label>
                        <input style={{width: 60, flex:1, marginLeft: 10}} value={TFGData[activeProfileID-1]["pcc"]} />
                    </div>
                </div>
                <div style={{marginTop: 10}}>
                    <RWTable 
                        columns={["Hours", "Weekday Factor", "Weeked Factor"]}
                        data={createTimeSeriesDataForWWGTable(TFGData[activeProfileID - 1])}
            ...
            <div>
                <SubPlot 
                    xArray={hourVals}
                    yData={TFGData[activeProfileID-1]["weekday_profile"]}
                    label={"WWG-WeekDay Profile"}
                />
                <SubPlot 
                    xArray={hourVals}
                    yData={TFGData[activeProfileID-1]["weekend_profile"]}
